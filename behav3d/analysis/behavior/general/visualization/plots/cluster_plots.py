...
Moved verbatim out of this package's ``__init__.py``. Living there made every
import of a *sibling* submodule (e.g. ``plots.proportion_bars``, which needs
only numpy/pandas/matplotlib/scipy) execute this module's ``import scanpy``
first -- ~6 s of the napari plugin's startup, paid via
``behav3d.analysis.behavior.state.utils`` for a single colour helper.
``__init__`` now re-exports these lazily; import them from here directly when
you can.
"""
import math
