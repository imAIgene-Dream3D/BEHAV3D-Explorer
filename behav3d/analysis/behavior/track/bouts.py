...
def apply_track_classifier_to_subtracks(
    ...
    if bool(plot_exemplars):
        ...
        if bool(plot_exemplar_backprojection_videos) or bool(plot_exemplar_backprojection_pdfs):
            coord_enrichment_video = _shared_ensure_exemplar_coordinate_columns(
                adata=adata_plot,
                output_dir=output_dir,
                cell_type=cell_type,
...
def run_state_based_analysis(
    ...
    if str(state_col) not in adata_filt.obs.columns:
        raise ValueError(
            ...
            f"but it is missing. Available columns (first 40): {list(adata_filt.obs.columns[:40])}"
        )
    ...
    if do_pca:
        ...
        adata_state_features = run_pca(
            adata_state_features,
            ncomps=min(
                int(adata_state_features.n_obs),
                int(adata_state_features.n_vars),
            ),
    ...
    if method in {"agglomerative_ward", "ward"}:
        method = "agglomerative"
    if method not in {"leiden", "agglomerative"}:
        raise ValueError("clustering_method must be one of: 'leiden', 'agglomerative'.")

    if method == "leiden":
        clustering_started = _vstart(
            verbose,
            "trajectory-clustering",
            "Leiden clustering | "
        ...
        _vdone(verbose, "trajectory-clustering", "Leiden clustering", clustering_started)
    else:
        linkage = str(agglomerative_linkage).strip().lower() or "ward"
        if linkage not in {"ward", "complete", "average", "single"}:
            raise ValueError("agglomerative_linkage must be one of: 'ward', 'complete', 'average', 'single'.")
        if int(n_clusters) < 2:
            raise ValueError("n_clusters must be at least 2 for agglomerative clustering.")
        if int(n_clusters) > int(adata_state_features.n_obs):
            raise ValueError(
                f"n_clusters={int(n_clusters)} exceeds available tracks ({int(adata_state_features.n_obs)})."
            )
        clustering_started = _vstart(
            verbose,
            "trajectory-clustering",
            f"Agglomerative clustering | linkage={linkage} | n_clusters={int(n_clusters)}",
        )
        X_cluster = (
            np.asarray(adata_state_features.obsm[resolved_leiden_use_rep], dtype=float)
            if str(resolved_leiden_use_rep) in adata_state_features.obsm
            else _to_numpy_2d(adata_state_features.X).astype(float, copy=False)
        )
        kwargs = {"n_clusters": int(n_clusters), "linkage": linkage}
        if linkage != "ward":
            kwargs["metric"] = str(leiden_metric)
        try:
    ...
    if "neighbors" not in adata_state_features.uns or "connectivities" not in adata_state_features.obsp:
        sc.pp.neighbors(
            adata_state_features,
            n_neighbors=int(n_neighbors),
            metric=str(leiden_metric),
            use_rep=resolved_leiden_use_rep,
            random_state=int(random_state),
        )
    sc.tl.umap(
    ...
    adata_state_features.uns["clustering"] = {
        "method": method,
        "cluster_key": str(cluster_key),
        "n_clusters": int(n_clusters) if method == "agglomerative" else None,
        "agglomerative_linkage": str(agglomerative_linkage) if method == "agglomerative" else None,
        "n_neighbors": int(n_neighbors),
        "leiden_resolution": float(leiden_resolution),
        "leiden_metric": str(leiden_metric),
